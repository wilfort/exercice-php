    ...
    if(empty($_POST['Note'])==false)
        {
            ...
            if(is_numeric($_POST['Note'])==true){
                switch ($_POST['Note']) {
                    case ((($_POST['Note'])<4)and($_POST['Note']>0)):
                        $message="Ce travail est nul.";
                        break;
                    case ((($_POST['Note'])<10)and($_POST['Note']>5)):
                        $message="Ce travail n'est pas terrible.";
                        break;
                    case (($_POST['Note'])==10):
                        $message="Tout juste!";
                        break;
                    case ((($_POST['Note'])<15)and($_POST['Note']>10)):
                        $message="C'est pas mal.";
                        break;
                    case ((($_POST['Note'])<19)and($_POST['Note']>14)):
                        $message="Bravo!";
                        break;
                    case ((($_POST['Note'])<21)and($_POST['Note']>18)):
